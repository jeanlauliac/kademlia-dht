...
//
function splitAndStore(contact, opt) {
    var newNode = exports.split(opt.bucket, opt.nth);
    if (opt.parent === null)
        this._root = newNode;
...
// `cb(err, added)`.
//
function invalidateAndStore(contact, bucket, validate, cb) {
...
// done.
//
exports.RoutingTable.prototype.store = function(contact, validate, cb) {
    ...
    if (!res.allowSplit || res.nth + 1 === id.BIT_SIZE)
        return invalidateAndStore(contact, res.bucket, validate, cb);
    splitAndStore.call(this, contact, res);
    return asyncCb(null, true);
};

// Find the bucket closest to the specified ID. Return an object containing
// `{parent, bucket, allowSplit}`
//
exports.RoutingTable.prototype.findBucket = function(contact) {
    var parent = null;
    var node = this._root;
    var allowSplit = true;
...
//
exports.split = function(bucket, nth) {
