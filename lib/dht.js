...
Dht.prototype._setNext = function (state, contact, others, cb) {
    var self = this;
    var payload = {id: this._id, key: state.key, value: state.value};
...
Dht.prototype._getFrom = function (id, contact, others, cb) {
    var payload = {id: this._id, targetId: id}
...
Dht.prototype._findNode = function (contact, targetId, cb) {
    var payload = {id: this._id, targetId: id};
    this.rpc.findNode(contact.endpoint, payload, function (err, result) {
        if (err) return cb(err);
...
Dht.prototype._validate = function (contact, cb) {
    this.rpc.ping(contact.endpoint, {id: this._id}, function (err) {
...
Dht.prototype._onPing = function (endpoint, payload, cb) {
    this._discovered(payload.id, endpoint, cb);
};
...
Dht.prototype._onStore = function (endpoint, payload, cb) {
    var self = this;
    this._discovered(payload.id, endpoint, function (err) {
        if (err) return cb(err);
