...
Dht.prototype.set = function (key, value, cb) {
    ...
    this._lookup (id, function (err, contacts) {
        if (err) return cb(err);
        self._setNext(id, key, value, contacts[0], contacts.slice(1), cb);
    });
};

Dht.prototype._setNext = function (id, key, value, contact, others, cb) {
    var self = this;
    this._rpc.store(this._id, contact.endpoint, key, value,
                    function (err) {
        if (others.length === 0) return cb();
        return self._setNext(id, key, value, others[0], others.slice(1), cb);
    });
}
...
Dht.prototype._getFrom = function (id, contact, others, cb) {
    this._rpc.findValue(this._id, contact.endpoint, id,
        ...
        if (err || !isValue) {
            if (others.length === 0) return cb();
            return _getFrom(id, others[0], others.slice(1), cb);
        }
...
Dht.prototype._lookup = function (id, cb) {
    ...
    var state = {
        list: new LookupList(contacts, Dht.BUCKET_SIZE),
        concurrency: 0,
        cb: cb
    }
    for (var i = 0; i < Dht.CONCURRENCY; ++i) {
        ++state.concurrency;
        this._lookupContact(state, list.get[i]);
    }
};
...
//
Dht.prototype._lookupContact = function (state, contact) {
    var self = this;
    this._rpc.findNode(this._id, contact.endpoint, id,
                       function (err, contacts) {
        if (!err) {
            state.list.update(contacts);
            var next = state.list.getNext();
...
Dht.prototype._discovered = function (contact, cb) {
    this._routes.store(contact, _validate.bind(this), cb);
};
