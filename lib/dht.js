...
Dht.spawn = function (rpc, seeds, cb) {
    console.error('start spawning on %s', rpc.endpoint);
...
Dht.prototype.bootstrap = function (seeds, cb) {
    ...
    function bootstrapSome(endpoint, err, res) {
        console.log('bootSome', endpoint, res.remoteId.toString(true));
        --remain;
        if (err) {
        ...
        self._routes.store(contact, function (err) {
            if (remain === 0) {
...
Dht.prototype.get = function (key, cb) {
    ...
    Lookup.proceed(id, seeds, this._lookupOpts, function (err, contacts) {
        if (err) return cb(err);
        return self._getFrom(id, contacts.shift(), contacts, cb);
    });
};

Dht.prototype._getFrom = function (id, contact, others, cb) {
    var payload = {id: this._routes.id, targetId: id};
    this.rpc.findValue(contact.endpoint, payload, function (err, result) {
        if (err || !result.value) {
            if (others.length === 0) return cb(null, void 0);
            return this._getFrom(id, others.shift(), others, cb);
        }
...
Dht.prototype._findNode = function (contact, targetId, cb) {
    ...
    this.rpc.findNode(contact.endpoint, payload,
                      function onNodeFound(err, result) {
        if (err) return cb(err);
        self._discovered(contact.id, contact.endpoint, function (err) {
            return cb(err, result.contacts);
        });
    });
};
...
Dht.prototype._validate = function (contact, cb) {
    this.rpc.ping(contact.endpoint, {id: this._routes.id},
                  function onPong(err, res) {
        if (err) return cb(null, false);
        if (!res.remoteId.equal(contact.id)) return cb(null, false);
        return cb(null, true);
    });
};
...
//
Dht.prototype._discovered = function (id, endpoint, cb) {
    if (!(id instanceof Id)) {
        return process.nextTick(function () {
...
//
Dht.prototype._onPing = function (endpoint, payload, cb) {
    var self = this;
    this._discovered(payload.id, endpoint, function onDiscovered(err) {
        if (err) return cb(err);
        cb(null, {remoteId: self._routes.id});
    });
};

// Store a key/value pair on the behalf of the specified `contact`.
//
Dht.prototype._onStore = function (endpoint, payload, cb) {
    var self = this;
    this._discovered(payload.id, endpoint, function onDiscovered(err) {
        if (err) return cb(err);
        self._cache[payload.key] = payload.value;
        cb();
    });
};
...
//
Dht.prototype._onFindNode = function (endpoint, payload, cb) {
    var self = this;
    this._discovered(payload.id, endpoint, function onDiscovered(err) {
        if (err) return cb(err);
        var res;
        try {
            res = self._routes.find(payload.targetId);
        } catch (err) {
            return cb(err);
        }
        return cb(null, {contacts: res});
    });
};
...
//
Dht.prototype._onFindValue = function (endpoint, payload, cb) {
    var self = this;
    this._discovered(payload.id, endpoint, function onDiscovered(err) {
        if (err) return cb(err);
        if (self._cache.hasOwnProperty(payload.key))
            return cb(null, {value: self._cache[payload.key]});
        var res;
        try {
            res = self._routes.find(payload.targetId);
        } catch (err) {
